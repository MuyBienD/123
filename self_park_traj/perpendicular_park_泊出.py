...
if __name__ == '__main__':
    ...
    num = 15
    alpha_arr = np.linspace(float(theta1), 0, num)
    print(alpha_arr)
    circle1_data = np.array(
        [(x_start - r_min * np.sin(alpha_arr)), (y_start - r_min + r_min * np.cos(alpha_arr)), alpha_arr])
    alpha_arr = np.linspace(np.pi / 2 - theta3, theta1, num)
    circle2_data = np.array(
        [(Circle2[0] + r_min * np.sin(alpha_arr)), (Circle2[1] - r_min * np.cos(alpha_arr)), alpha_arr])
    alpha_arr = np.linspace(0,theta3, num)
    circle3_data = np.array(
        [(circle3[0] - r_min * np.cos(alpha_arr)), (circle3[1] + r_min * np.sin(alpha_arr)), (np.pi / 2 - alpha_arr)])
    adjust_data = np.array([[x_end] * num, np.linspace(y_end, y_end - 4, num)[::-1], [np.pi / 2] * num])

    data_traj = np.concatenate((adjust_data, circle3_data, circle2_data, circle1_data), axis=1)
