...
if __name__ == '__main__':
    ...
    num = 20
    alpha_arr = np.linspace(0, 90 * np.pi / 180, num)
    circle1_data = np.array(
        [x_start - r_min * np.sin(alpha_arr), y_start - r_min + r_min * np.cos(alpha_arr), alpha_arr])

    plt.figure()
    plt.gca().set_aspect('equal', adjustable='box')
    for item in range(len(circle1_data[0])):
        c_o = np.array([circle1_data[0][item], circle1_data[1][item]])
        plt.plot(c_o[0], c_o[1], "g.")
        car = car_p(car_length, car_width, c_o)
        new_car = RotTheta(car, circle1_data[2][item], c_o)
    ...
    # circle3_data = np.array([np.linspace(x_end, x_end + 1, num), [y_end] * num, [0] * num])
    # data_traj = np.concatenate((circle1_data, circle2_data, circle3_data), axis=1)
    # plt.figure()
    # plt.gca().set_aspect('equal', adjustable='box')
